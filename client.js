...
var ASM_CONSTS = {
 ...
 9413360: $0 => {
  ...
  try {
   ...
   if (!audio_data) throw "Audio data not found";
 ...
 try {
  ...
  case 4352:
   /* AL_FORMAT_MONO8 */ if (size > 0) {
    ...
    for (var i = 0; i < size; ++i) {
     channel0[i] = HEAPU8[pData++ >>> 0] * .0078125 - /* 1/128 */ 1;
    }
   }
  ...
  case 4354:
   /* AL_FORMAT_STEREO8 */ if (size > 0) {
    ...
    for (var i = 0; i < size >> 1; ++i) {
     channel0[i] = HEAPU8[pData++ >>> 0] * .0078125 - /* 1/128 */ 1;
     channel1[i] = HEAPU8[pData++ >>> 0] * .0078125 - /* 1/128 */ 1;
    }
   }
   buf.bytesPerSample = 1;
   buf.channels = 2;
  ...
  case 4355:
   /* AL_FORMAT_STEREO16 */ if (size > 0) {
    ...
    for (var i = 0; i < size >> 2; ++i) {
     channel0[i] = HEAP16[pData++ >>> 0] * 30517578125e-15;
     /* 1/32768 */ channel1[i] = HEAP16[pData++ >>> 0] * 30517578125e-15;
    }
   }
   buf.bytesPerSample = 2;
   buf.channels = 2;
...
function _alcGetIntegerv(deviceId, param, size, pValues) {
 ...
 switch (param) {
 case 4096:
  /* ALC_MAJOR_VERSION */ HEAP32[((pValues) >>> 2) >>> 0] = 1;
  break;

 case 4097:
  /* ALC_MINOR_VERSION */ HEAP32[((pValues) >>> 2) >>> 0] = 1;
  break;
...
var ___start_em_js = Module["___start_em_js"] = 9417350;

var ___stop_em_js = Module["___stop_em_js"] = 9418440;
